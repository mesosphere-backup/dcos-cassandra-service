...
- For `node replace` to work appropriately upon loss of an agent, you must wait until `TASK_LOST` is sent by Mesos for the node you're replacing. See the `--agent_reregister_timeout` flag on the Mesos masters to see how long until `TASK_LOST` is sent after the agent fails.
